...
VkImageView Application::createImageView(VkImage img, VkFormat format) {
...
void Application::createTextureSampler() {
    ...
    if (vkCreateSampler(_device, &samplerInfo, nullptr, &_textureSampler) != VK_SUCCESS) {
        throw std::runtime_error("failed to creat etexture sampler!");
    }
...
void Application::transitionImageLayout(VkImage img, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
    ...
    if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED &&  VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
...
void Application::copyBufferToImage(VkBuffer buffer, VkImage img, uint32_t width, uint32_t height) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
